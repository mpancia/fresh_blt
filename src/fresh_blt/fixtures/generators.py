...
class BLTGenerators:
    ...
    def small_election(self) -> str:
        ...
        return self.faker.blt_content(
            self.faker.election(num_candidates=4, num_ballots=25)
        )

    def large_election(self) -> str:
        """Generate a large election with many candidates and ballots."""
        return self.faker.blt_content(
            self.faker.election(num_candidates=8, num_ballots=150)
        )
...
def generate_test_election(num_candidates: int = 4,
                         num_ballots: int = 50,
